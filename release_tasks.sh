...
&& DJANGO_SETTINGS_MODULE='atlas.settings.demo' celery -A atlas worker -l DEBUG -E --detach \
...
&& python  manage.py  shell --command="import atlas; from etl.tasks.search import reports; reports.reset_reports.delay()" \
&& python  manage.py  shell --command="import atlas; from etl.tasks.search import terms; terms.reset_terms.delay()"
