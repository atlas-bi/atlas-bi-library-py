...
import type { InitiativeUpsertInput } from '@/actions/initiatives-actions'
import { upsertInitiativeAction } from '@/actions/initiatives-actions'
import type { AtlasInitiative } from '@/lib/atlas-api'
...
export function InitiativeForm({
  ...
  function submit() {
    const payload: InitiativeUpsertInput = {
      initiativeId: initial?.initiative_id,
      name,
      description
    }

    startTransition(async () => {
      await upsertInitiativeAction(payload)
    })
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      <h1 className="text-4xl font-light tracking-tight text-gray-900 mb-2">
        {initial ? `Editing ${initial.name}` : 'New Initiative'}
      </h1>

      <div className="flex justify-between items-center mb-6">
        <Link
          href={
            initial ? `/initiatives/${initial.initiative_id}` : '/initiatives'
          }
      ...
      {!session ? (
        <div className="rounded border bg-yellow-50 p-4 text-sm text-yellow-900 mt-6">
          You must be logged in to edit initiatives.
        </div>
      ) : null}
    </div>
  )
