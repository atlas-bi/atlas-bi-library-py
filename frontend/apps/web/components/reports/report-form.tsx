...
import type { ReportUpsertInput } from '@/actions/reports-actions'
import { upsertReportAction } from '@/actions/reports-actions'
import type { AtlasReport } from '@/lib/atlas-api'
...
export function ReportForm({
  ...
  function submit() {
    const payload: ReportUpsertInput = {
    ...
    startTransition(async () => {
      await upsertReportAction(payload)
    })
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      <h1 className="text-4xl font-light tracking-tight text-gray-900 mb-2">
        {initial ? `Editing ${initial.title || initial.name}` : 'New Report'}
      </h1>

      <div className="flex justify-between items-center mb-6">
        <Link
          href={initial ? `/reports/${initial.report_id}` : '/reports'}
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            placeholder="e.g Monthly Admissions Dashboard"
            required
          />
        </div>
