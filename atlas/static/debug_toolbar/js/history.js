...
function refreshHistory() {
  ...
  ajaxForm(formTarget)
    .then(function (data) {
      ...
      container.querySelectorAll('tr[data-store-id]').forEach(function (node) {
        node.remove();
      });
    ...
    .then(function (refreshInfo) {
      refreshInfo.newIds.forEach(function (newId) {
        const row = container.querySelector(`tr[data-store-id="${newId}"]`);
        row.classList.add('flash-new');
      });
      setTimeout(() => {
        container.querySelectorAll('tr[data-store-id]').forEach((row) => {
